        ...
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php 
          $query = mysqli_query($conn,"SELECT * FROM news where account_type = 'registerar' order by id");
            while($row = mysqli_fetch_assoc($query) >= 1){
              $up_on = $row['updated_on'];
              if ($up_on == null) {
                $qry = mysqli_query($conn,"SELECT * FROM news where account_type = 'registerar' order by id desc");
              }else {
                $qry = mysqli_query($conn,"SELECT * FROM news where account_type = 'registerar' order by updated_on desc");
              }
            while($rows = mysqli_fetch_assoc($qry)){
                  ...
                  $query = mysqli_query($conn, "SELECT * FROM users_account where user_id = '$posted_by'");
...
<p><?php echo $accountType;?><span class="time-right">
    <?php 
         if ($updated_on == null) {
              echo $time_ago;
            }else {
