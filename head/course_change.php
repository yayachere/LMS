...
if (isset($_POST['register'])) {   
    ...
    $queryAssignedCourse = mysqli_query($conn, "SELECT * FROM assigned_course WHERE year = '$year' and semister = '$semister' and program = '$program' and department = '$department' and course_title = '$courseTitle' and section = '$section' and instructor = '$instructor'") or die(mysqli_error($conn));
    ...
    if ($queryNumRow > 0) {
      ...
      $sql = mysqli_query($conn, "UPDATE assigned_course SET course_title = '$courseTitle', course_code = '$courseCode', instructor = '$instructor',section = '$section' WHERE id = '$id'") or die(mysqli_error($conn));

       if($sql == true) {

        echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
            <i class="bi bi-check-circle-fill me-1"></i>
            Successfully updated assigned course!!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
       }
       else {
        ...
        echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
            <i class="bi bi-check-circle-fill me-1"></i>
            Failed to update assigned course, please try again!!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
       }
...
<form action="#" method="post"  enctype="multipart/form-data">
    ...
    <div class="form-floating col-md-12 mt-3 mb-3">
          <select class="form-select" name="courseTitle" id="courseTitle" aria-label="Floating label select example" required>
            <option value="<?php echo $courseTitle; ?>" selected><?php echo $courseTitle;?></option>
            <?php
              $sql = mysqli_query($conn, "SELECT * FROM courses where department = '$department' and year = '$year' and semister = '$semister' and program = '$program' and course_title != '$courseTitle'") or die(mysqli_error($conn));
    ...
    <div class="form-floating col-md-12 mt-3 mb-3">
          <select class="form-select" name="instructor" id="instructor" aria-label="Floating label select example" required>
            <option value="<?php echo $instructor; ?>" selected><?php echo $instructor;?></option>
            <?php
    ...
    <div class="form-floating col-md-12 mt-3 mb-3">
          <select class="form-select" name="section" id="section" aria-label="Floating label select example" required>
            <option value="<?php echo $section; ?>" selected>Section <?php echo $section; ?></option>
